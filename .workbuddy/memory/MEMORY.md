...
- **远端**：`origin` = https://github.com/ysyonline/PvZlite（public）。本地分支已由 `master` 改名为 **`main`**（对齐远端默认分支）；`v1.0.0`/`v1.1.0` 标签均已推。**推送必须用 helper 覆盖**（见下「推送命令」），否则会挂在 PortableGit 的 `helper-selector` 上。**v1.1 已定版**：`v1.1.0` 标签打在 `fc7b982` 并已推远端（22:1x，ls-remote 权威核验一致）。小坑（本机已实测两次）：推送后 origin/main 引用可能显示 `[ahead N]` 甚至 `[gone]`——**推送本身是成功的**，用 `git ls-remote origin main` 核验即真；若 fetch/update-ref 修不回来，检查 `.git/packed-refs` 是否乱序（必须字典序：heads → remotes → tags），手排后即恢复
