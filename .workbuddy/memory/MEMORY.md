...
## 当前状态（2026-09-19 06:22 更新）
- **远端已同步**（用户 06:21 授权「推」，ls-remote 核验一致）：origin/main=`18677e5`（v1.2.1 发布包刀）· tag `v1.2.1`=`f5d660d` 已推远端；工作树全净，本地零领先
- **v1.2.1 正式版已发**：tag 锚定 `f5d660d`（源码考古更正：v1.2.1 早已在 `6da6b2c` 入库，「未提交」系过时信息）；发布包 `production/release/v1.2.1/`（三件套+冻结副本，Go 判定）；四门控按 v1.2.1 基线复跑全绿（26/56/56/bench PASS）
- 优化池 O1/O2/O4 🧊 冻结继承不变；N-13/N-14 冻结不变
- **v1.3 屋顶关开发内容在 main 已合入未定版**（`e7a6a1e`）；v1.2.1 与 v1.3 线并行无冲突。**下一步 = v1.3 人工验收 M1-M8（真机过 ?level=5、投手弧线、9 卡视觉）→ 定版 v1.3**
...
- **屋顶线（L5 屋顶，v1.3 候选）：立项 + S1 地基与护栏完成（09-18/19 夜间，G1/G2 双门 PASS）**
