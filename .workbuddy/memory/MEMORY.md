...
## 📌 下班交接（2026-09-21 17:0x，换电脑/新会话续干从这里读）

- **本地=远端（main=`a85c769`），工作树干净，无挂起改动**（基点以 `git log --oneline -1` 实测为准）。v1.4.0 已发布（tag `v1.4`=`d883d6c`），三件套评审已收官入库
- **下一刀 = v1.5 新植物三件套施工**（评审定稿 `production/v15-trio-review.md`，四项拍板唯一依据）
  - 五刀切分：S1 布局 B2 两常量（gh 120→104 + DECK_SLOTS.y0 430→470）+ 卡槽栏 cw 98→92 修横向溢出 + REG-SLOT-04 补 10 槽右缘断言 → S2 三卡 CARDS 注册 + DIFF_AWARD 扩表（困难L3→corn/困难L4→snowpea/地狱L5→icemelon）+ per-difficulty 存档键 → S3 corn 弹道（复用 melon 直线分支）→ S4 chill 状态机（z.slowT + L1520 速度 ×0.6 + dt 帧驱动【禁定时器】+ drawZombie 染色）→ S5 icemelon + code-map 重跑
  - 数值表/slow 三铁律/回退预案/明确不做项（热键'0'/投手CD/grandfather）全在 review 文档 §二
  - ⚠️ review 文档 L 号为 v1.4.0 冻结口径（3000 行），S5 收尾 code-map 重跑后行号全位移
  - 种植 CD 6/6/10 与 dur 900/750/1200 为【假设初值，真机验收调参】
- 换机操作：`git pull`（先开加速器）→ 读 `2026-09-21.md` 16:2x/16:4x 节 + `production/v15-trio-review.md` → 按 S1 开工

## 📌 旧交接（2026-09-20 23:0x，历史参考）
