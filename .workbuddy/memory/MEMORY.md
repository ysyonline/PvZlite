...
## 版本线与指纹
v1.5 → v1.6.0（tag v1.6）→ v1.7.0（`5ff52d2b…14f6`/162,685B/3289行）→ v1.8.0（`889c0cb0…804f`/163,493B/3291行）→ **v1.9.0 已发·全链路闭环**（源码权威 `8963ff1`，发布包 `c893120`，tag v1.9.0 已推；LF SHA-256 `213929e6…d868`/164,063B/3289行）→ **v2.0.0 已发·全链路闭环（09-24）**（源码权威 `b22debe`，发布包 `2a8c38e`，tag v2.0.0@`4846dcc` 已推；LF SHA-256 `e8cb90b3…edbb0`/190,847B/3618 行）→ **v2.0.1 修复方案已拍板·待施工**
...
- **v2.0.1 待施工（09-24 17:3x 拍板，方案完整无阻塞）**：test=1 只解「锁定」闸·**占位 toast 保留**——源码 3 处 `!testMode` 前置（onClickSelect L1272 解锁检查 / onClickEnd L1319 / drawSelect L3441 locked 视觉）+ VERSION L61→v2.0.1；测试侧零迁移（v20-select-draw 无锁定暗色断言、menu-nav 不带 test=1、REG-TESTMODE-01 无新写档路径）；新增自证 v20-test-unlock.js（?test=1 点真卡锁定关 1-4 进 deck + 点 1-7 占位 toast+deny + 旧源 v2.0.0 必红）；定版通扫 5 处活默认值（prelude.js L24/L52、v17-acc L119、v18-acc L26/L53）；发布包 v2.0.1 四件套+tag 需用户授权
- 遗留：R5 弹道手感人工复核
