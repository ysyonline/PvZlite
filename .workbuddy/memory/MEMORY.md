...
## 当前状态
- **v2.2.4 奖励展示简化完成**（2026-09-26，未提交）：用户拍板去掉礼盒特效，通关奖励植物改为**淡入展示**（alpha 0→0.85 + 轻上浮，约 1s）。礼盒全部绘制代码移除（-190 行），GIFT_OPEN_TIME/GIFT_REVEAL_TIME 常量删除，音效简化为单次 sun 叮。门控全绿（FULL 91/91 · BUS 56/56 · 视觉 4/4 · 判别力 v224 先红后绿）。**注意：v2.2.3 与 v2.2.4 两轮改动均在未提交状态（工作区未 commit）**
- **v2.2.3 礼盒 P1 打磨完成**（2026-09-26，未提交，**已被 v2.2.4 方向推翻**）：四项 P1（盖子弹跳/四角星粒子/闪光环光柱/音效联动）曾落地并门控全绿，但用户随后拍板去礼盒特效 → 本轮代码已整体移除。历史判别力脚本 v222/v223 已退役（判据失效），v224 淡入判别力接替
...
- **欠账（下一刀）**：礼盒视觉欠账闭环后用户转向"去礼盒"（v2.2.4 落地）。v2.3 候选入口（下一开窗点）：世界 3（墓地）/ 世界 4（房屋）开放 · 墓碑机制实装 · 金币留存钩子
