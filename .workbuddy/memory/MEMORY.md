...
## 版本线
v1.5（`56b8d25`）→ v1.6.0（定版 `0b969d5`，tag v1.6 ✅）→ v1.7.0（源码权威 `c379872`，发布包 `988b69f`，tag v1.7.0 已推 ✅）→ **v1.8.0 已发布**（源码权威 `c10f4a1`，发布包 `20d33e1`，tag v1.8.0 已推 ✅）→ **v1.9.0 在途·收口+测量版**（UI 修复 `ebf352f` + 行尾 `f20057e` + 立项刀 `f986124`/GRID 采纳 `9cc06f7`/R10 `df4b0e1`/R11 `72ecef6`/plan 定稿 `b5a9438`，main=`b5a9438` 已推；**L49=v1.9.0-wip 未定版**，定版刀待启动）
★ 冻结指纹（LF）：v1.7.0 `5ff52d2b…14f6` / 162,685 B / 3289 行 · **v1.8.0 `889c0cb0…804f` / 163,493 B / 3291 行**（历史在日志）

## 当前状态（09-23 14:0x · 公司机 user3667 · **v1.9.0 在途：立项+测量五刀已推（main=`b5a9438`），待定版刀**）
- **v1.9 收口+测量范围全部闭环并已推送（用户 13:3x 授权提交推送）**：立项刀 `f986124`（L49→v1.9.0-wip + 活默认值通扫 4 处）→ GRID 采纳 `9cc06f7`（Q-2=A，超差事实保留）→ R10 三件 `df4b0e1` → R11 三件 `72ecef6` → plan 定稿 `b5a9438`（§8 记 R10/R11 摘要）；远端 main=`b5a9438` 双确认；四门控复跑全绿（SMOKE 29/BUS 56/FULL 87/bench PASS）
- **R10 核心数**：自然堆叠机会率 cabbage 1.71%/corn 4.33%/melon 1.21%；P(≥1|n≥2) cabbage=1(52)/corn=1(177)/melon=0.5385；vCross=0 全种子（同格锁契约保持）；判别力自证 旧源 same=5,cross=2 vs 新源 same=17,cross=0。**R11 核心数**：利用率上界 81.8/71.5/64.3% vs clampRate≈0（R7 预警定量实锤）；32 对零结局翻转（溅射价值=提速+补刀）
- **遗留待办**：①R7 脚本 `v18-splash-distribution.js` 读 `e.w.kind` 在 prelude hitKind 修复后复跑必红（潜在回归，R10 报告 §5 登记，待拍板修）②v1.9 定版刀未做（L49 切 v1.9.0 + 发布四件套 + tag）③R5 弹道手感人工复核维持建议本版不做
