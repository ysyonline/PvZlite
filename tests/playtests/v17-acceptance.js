...
// 验收两刀：
//   R-B  cabbage 溅射（本版新机制，必测）：splash 30px·40% ⇒ 溅射伤害 8；溅射不定身；
//        同排两僵尸受控构造（A 直中 |Δx|<42 / B 距命中点 <30）⇒ B 掉血 8 且 freezeT===0；
//        对照臂 间距 >30 ⇒ B 不掉血；corn/melon/icemelon 溅射数值未变（6 / 35.75 / 35.75）。
...
(async () => {
  ...
  const helper = `(function(){
    ...
    // 受控命中：把真实弹体摆到命中点，构造同排 A/B 两只僵尸，跑一帧 updateProjectiles
    window.__h.hitPair = function(pr, row, ax, bx){
      ...
      return {A_lost:1000-A.hp, B_lost:1000-B.hp,
  ...
  // ==========================================================================
  // env · 版本自证（确保验收对象是 v1.7 源码）
  // ==========================================================================
  const VER = await evalPage('VERSION');
  push('env','验收对象版本自证', String(VER).indexOf('1.7')>=0, [
    'VERSION='+JSON.stringify(VER)+'（期望含 1.7）',
  ...
  // ---- R-B-2 · 端到端：A 直中(20) + B 溅射(8) 且 B.freezeT===0（溅射不定身）----
  // ---- R-B-3 · 对照臂：B 距命中点 >30（且 >42 直中窗）⇒ B 不掉血 ----
  const RB2 = await evalPage(`(function(){
    window.__h.freeze(); level=LEVELS[1]; levelNo=1; DIFF='normal';
    const row=2;
    const pr1=window.__h.realPr('cabbage',2,row,700);       // x=400 命中点
    const hit=window.__h.hitPair(pr1, row, 415, 425);        // A|Δx|=15 直中 · B|Δx|=25<30 溅射
    const pr2=window.__h.realPr('cabbage',2,row,700);
    const ctrl=window.__h.hitPair(pr2, row, 415, 445);       // A 直中 · B|Δx|=45>42 且 >30 ⇒ 不命中
    return {hit:hit, ctrl:ctrl, splash:pr1.splash};
  })()`);
  {
    const h=RB2.hit, c=RB2.ctrl;
    const c1 = Math.abs(h.A_lost-20)<1e-9;
    const c2 = Math.abs(h.B_lost-8)<1e-9;                    // 20×0.40
    const c3 = h.B_freezeT===0;                              // 溅射不定身
    const c4 = Math.abs(c.A_lost-20)<1e-9 && Math.abs(c.B_lost-0)<1e-9;
    const pass = c1 && c2 && c3 && c4;
    push('R-B-2','cabbage 溅射端到端：同排 A 直中(20) + B 溅射(8=20×0.40) 且 B.freezeT===0；对照臂 B(>30px) 不掉血', pass, [
      '命中点 x≈400（真机 cabbage 弹体摆位）：A@415(|Δx|=15<42 直中) · B@425(|Δx|=25<30 溅射)',
      'A 掉血='+(+h.A_lost).toFixed(1)+'(期望20 直中) · B 掉血='+(+h.B_lost).toFixed(1)+'(期望8=20×0.40) → '+(c1&&c2?'OK':'异常'),
      'B.freezeT='+h.B_freezeT+'(期望0，口径：溅射不施加控制) · A.freezeT='+h.A_freezeT+'(cabbage 无控制，期望0) → '+(c3?'OK':'异常'),
      '对照臂：A@415 直中 · B@445(|Δx|=45>42 且 >30) ⇒ A 掉血='+(+c.A_lost).toFixed(1)+'(期望20) B 掉血='+(+c.B_lost).toFixed(1)+'(期望0) → '+(c4?'间距隔离生效':'异常'),
      '源事实：updateProjectiles L1585 `Math.abs(z2.x-pr.x)<pr.splash` 一维同排带；直中框 `|z.x-pr.x|<42` 保证 A 先结算并 break'
    ]);
  }

  // ---- R-B-4 · corn/melon/icemelon 溅射数值未变（6 / 35.75 / 35.75）+ icemelon 溅射减速 ----
  const RB4 = await evalPage(`(function(){
    ...
    function run(type, bx, ax){
      ...
      const info={type:type, dmg:pr&&pr.dmg, splash:pr&&pr.splash, ratio:pr&&pr.splashRatio,
                  hasChill:!!(pr&&pr.chill), hasButter:!!(pr&&pr.butter)};
    ...
    return {
      corn:     run('corn',     425),   // |Δx|=25<30(溅射)
      melon:    run('melon',    440),   // |Δx|=40<55(溅射)
      icemelon: run('icemelon', 440)    // |Δx|=40<55(溅射)
    };
  })()`);
  {
    let pass=true; const notes=[];
    const c1 = Math.abs(RB4.corn.A_lost-15)<1e-9 && Math.abs(RB4.corn.B_lost-6)<1e-9 && RB4.corn.splash===30 && Math.abs(RB4.corn.ratio-0.40)<1e-9;
    if(!c1) pass=false;
    notes.push('corn：直中='+(+RB4.corn.A_lost).toFixed(1)+'(期望15) 溅射='+(+RB4.corn.B_lost).toFixed(1)+'(期望6=15×0.40) splash='+RB4.corn.splash+'/ratio='+RB4.corn.ratio+' → '+(c1?'未变':'异常'));
    const c2 = Math.abs(RB4.melon.A_lost-65)<1e-9 && Math.abs(RB4.melon.B_lost-35.75)<1e-9 && RB4.melon.splash===55 && Math.abs(RB4.melon.ratio-0.55)<1e-9;
    if(!c2) pass=false;
    notes.push('melon：直中='+(+RB4.melon.A_lost).toFixed(1)+'(期望65) 溅射='+(+RB4.melon.B_lost).toFixed(2)+'(期望35.75=65×0.55) splash='+RB4.melon.splash+'/ratio='+RB4.melon.ratio+' → '+(c2?'未变':'异常'));
    const c3 = Math.abs(RB4.icemelon.A_lost-65)<1e-9 && Math.abs(RB4.icemelon.B_lost-35.75)<1e-9 && RB4.icemelon.splash===55 && Math.abs(RB4.icemelon.ratio-0.55)<1e-9;
    const c4 = RB4.icemelon.B_slowT>=1.9;                    // 溅射也施加 chill（v1.5 决议1 全命中减速）
    if(!(c3&&c4)) pass=false;
    notes.push('icemelon：直中='+(+RB4.icemelon.A_lost).toFixed(1)+'(期望65) 溅射='+(+RB4.icemelon.B_lost).toFixed(2)+'(期望35.75) splash='+RB4.icemelon.splash+'/ratio='+RB4.icemelon.ratio+' → '+(c3?'未变':'异常'));
    notes.push('icemelon 溅射邻体 B slowT='+(+(RB4.icemelon.B_slowT||0)).toFixed(3)+'(期望≈2.0，溅射也减速) → '+(c4?'OK':'异常'));
    notes.push('corn 溅射邻体 B.freezeT='+RB4.corn.B_freezeT+'(期望0，溅射不定身) → '+(RB4.corn.B_freezeT===0?'OK':'异常'));
    push('R-B-4','corn/melon/icemelon 溅射数值未变（6 / 35.75 / 35.75）· icemelon 溅射邻体仍减速', pass, notes);
  }
  ...
  const RB_vis = await evalPage(`(function(){ window.__h.freeze(); level=LEVELS[1]; levelNo=1; DIFF='normal';
    ...
    const A=window.__h.mkZ(row, 415, 1000, 0), B=window.__h.mkZ(row, 425, 1000, 0);
  ...
  await shot('v17-cabbage-splash.png');
  {
    ...
    push('R-B-5','cabbage 溅射视觉证据：飞行帧 + 命中帧（含爆点粒子）', pass, [
      ...
      '命中帧：A 掉血='+RB_vis.A_lost+'(期望20) B 掉血='+RB_vis.B_lost+'(期望8) · effects(爆点粒子)='+RB_vis.effects+'(期望>0)',
