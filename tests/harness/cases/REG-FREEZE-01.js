/* REG-FREEZE-01 · v1.6/v1.7 corn 黄油定身：27% 黄油命中 ⇒ 完全定身 3.0s（移动+啃食双停）· 2026-09-22 V16-QA-1
 * ============================================================================
 * 契约（v1.6，与工程侧共用同一份）：
 *   · corn 弹体有 27% 概率为「黄油弹（butter）」；命中僵尸 ⇒ **完全定身 3.0s**（移动 + 啃食 **双停**）。
 ...
 *      ⇒ 27% 分支**确定性可复现**。每个场景用 `findFreeze()` 扫描 seed 1..120，取「首个在该
 ...
 *      各场景**独立扫种子**（不跨场景复用同一发序假设）⇒ 对「发射时 vs 命中时掷 27%」两种实现均稳。
 *   3) 时钟走 `g.__updateRaw(0.05)`（不累 gt ⇒ 隔离波次刷怪，纯定身观测）。
 * 断言：§1 黄油命中 freezeT≈3.0（区间式，命中同帧可能已递减）；
...
module.exports = {
  id: 'REG-FREEZE-01',
  name: 'corn 黄油定身：27% 黄油命中完全定身 3.0s（移动+啃食双停 / 独立并存 / 到期恢复）',
  seed: 42,
  run({ game: g, assert }) {
    const S = g.sandbox;
    const K = S.__consts;
    ...
    // ═══ §1 黄油命中 ⇒ freezeT≈3.0（区间式：命中同帧可能已按 dt 递减）═══
    // ═══ §0 隐式：27% 分支确定性可复现（否则 findFreeze 返回 null）═══
    const F1 = findFreeze('freeze-f1', () => { plantCorn(1, 2); return { z: mkZ(400, 2, { hp: 1e6 }) }; }, 10);
    assert(F1, '§0/§1：27% 黄油分支应在 seed 1..120 内确定性可复现命中（契约 27%）', null);
    assert(F1.freezeT > 2.9 && F1.freezeT <= 3.0,
      '§1：黄油命中后 freezeT≈3.0（区间式 >2.9 && <=3.0）', F1.freezeT);
    ...
    const zM = F2.z;
    assert(F2.freezeT > 2.9 && F2.freezeT <= 3.0, '§2：定身起始 freezeT≈3.0', F2.freezeT);
    const xFrozen = zM.x;
    const walkFrozen = zM.walk;                       // 完全定身 ⇒ walk 动画亦不推进（源码 `if(!frozen)z.walk+=...`）
    for (let i = 0; i < 40; i++) {                    // 2.0s（<3.0s 定身窗）
      g.__updateRaw(0.05);
    ...
    // ═══ §4 定身结束（>3.0s）恢复移动 ═══
    runDt(1.6);                                       // 累计 >3.0s（§2 已走 2.05s + 1.6s > 3.0s 定身窗）
    ...
    // 假设（契约口径）：黄油在**发射时**掷定并标记在弹体上（「弹体有 27% 概率为黄油弹」）；
    //   ⇒ 无黄油标记的普通 corn 弹体命中恒不产生 freezeT。若工程改为「命中时掷 27%」，本组需同步调整。
    g.seed(42);
