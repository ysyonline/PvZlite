 ...
 *   ① 屋顶格无盆点投手（100 费卡同样被拦）→ 拒绝零副作用（不扣费/不进 CD/保留选中）
 *   ② 屋顶格放盆 → 成功；盆上点花盆 → 拒（已占用）
 *   ③ 盆上连种投手 → 成功且投手可开火（cabbage 分支挂上 updatePlant，真实发射验证）
 *   ④ 铲子最上层语义：盆+植物 → 铲植物留盆返 50（100/2）；空盆 → 铲盆返 25
...
module.exports = {
  ...
  run({ game: g, assert }) {
    ...
    assert(p.plants === 0 && p.sun === 999 && !(p.cardCD.cabbage > 0) && p.selected && p.selected.type === 'cabbage',
      '① 无盆点投手应拒绝：零副作用且保留选中（E1 含 100 费卡）',
    ...
    assert(p.plants === 2 && p.sun === 874,
      '③ 盆上连种投手成功：plants=2 且扣 25+100', [p.plants, p.sun, p.plantsArr]);
    ...
    // —— ④ 铲子最上层语义（E4）：盆+植物 → 铲植物留盆，返一半卡价 50 ——
    g.selectShovel();
    g.clickGrid(2, 2);
    p = g.probe();
    assert(p.plants === 1 && p.plantsArr[0].type === 'planter',
      '④ 铲盆+植物：应移植物留盆（最上层语义）', p.plantsArr);
    assert(p.sun === 874 + 50, '④ 铲投手应返一半卡价 +50（100/2）', p.sun);
    // 空盆 → 铲盆返 25（铲子用后即清，重选）
    g.selectShovel();
    g.clickGrid(2, 2);
    p = g.probe();
    assert(p.plants === 0, '④ 空盆可被铲除', p.plants);
    assert(p.sun === 924 + 25, '④ 铲花盆应返 +25（25 保底）', p.sun);
