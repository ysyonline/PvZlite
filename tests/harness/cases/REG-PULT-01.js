 ...
 *   ① 静止目标（spd=0, eating=true）：按 §3.4 解算 vy0 后注入 cabbage 弹，命中应扣 20。
 *   ② 移动目标（spd=16 normal）：按「预测提前量」解算（lead = 目标x − spd·t0）后发射，命中应扣 20。
...
module.exports = {
  ...
  run({ game: g, assert }) {
    ...
    g.sandbox.__projectiles.push({ x: spawnX, y: zy, vx: VX, vy: vy0s, g: GG, dmg: 20, row: R, type: 'cabbage', splash: 0, dead: false });
    let guard = 0;
    while (g.probe().projectiles > 0 && guard < 600) { g.tick(0.01); guard++; }
    assert(Math.abs(sz.hp - (180 - 20)) < 1e-6, '① 静止目标注入 cabbage 弹应命中扣 20', sz.hp);
    ...
    g.sandbox.__projectiles.push({ x: spawnX, y: zy, vx: VX, vy: vy0m, g: GG, dmg: 20, row: R, type: 'cabbage', splash: 0, dead: false });
    guard = 0;
    while (g.probe().projectiles > 0 && guard < 900) { g.tick(0.01); guard++; }
    assert(Math.abs(mz.hp - (180 - 20)) < 1e-6, '② 移动目标按预测提前量发射应命中扣 20（接近必中）', [mz.hp, mz.x]);
  },
};
