 ...
 *   2. 特效颜色随阶位（bucket=gold 色 #ffd54a / normal=bronze 色 #b87333）
...
module.exports = {
  ...
  run({ game: g, assert }) {
    ...
    assert(fx[0].color === C.TIER.gold.color,
      '金光颜色应随阶位（bucket→gold 色）', fx[0].color);
    assert(coins.every(c => c.color === C.TIER.gold.color),
      '飞币颜色应随阶位（bucket→gold 色）', coins[0] && coins[0].color);
    ...
    fx = g.probe().effectsArr.filter(e => e.kind === 'coin-flash');
    assert(fx.length === 1 && fx[0].color === C.TIER.bronze.color,
      '普通怪金光应为铜色', fx[0]);
