...
module.exports = {
  ...
  run({ game: g, assert }) {
    ...
    assert(rec.oscStarted === 0, 'suspended 时振荡器不得直接启动（会被丢弃）', rec.oscStarted);
    assert(p.audioQueueLen === 1, '种植音效应进入补播队列', p.audioQueueLen);

    // ---- 2) 再点阳光（也挂起入队）----
    g.selectCard(1);
    g.clickGrid(1, 2);
    p = g.probe();
    assert(p.audioQueueLen === 2, '第二个音效也应入队', p.audioQueueLen);
    assert(rec.oscStarted === 0, '仍不得直接启动', rec.oscStarted);
    ...
    assert(rec.oscStarted === 2, '两个挂起的种植音效应全部补播', rec.oscStarted);
    ...
    assert(rec.oscStarted === 3, '地瓜种植音效应直接发声', rec.oscStarted);
  },
};
