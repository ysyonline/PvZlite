...
| 2026-09-23 | **v2.0 T-104a harness 双世界兼容层（M1 首施工单元）**：`tests/harness/index.js` PROBE_SUFFIX 内新增——①`setLevel`/`setUnlocked` **双签名**（数字 1..5 ↔ `'w-l'` 键，锚点映射 `__ANCHOR`/`__ANCHOR_REV` 出处 plan §3.3 / Q-14；v1 数字路径逐字节保留，失败 `return false` 且状态不变；`setUnlocked` v2 语义 = 最高解锁键，Q-16）；②`probe` **双暴露** `levelKey`（v1 源由 levelNo 经锚点派生）/ `levelNo`（v2 源由 LEVEL_INDEX 序位派生），均 typeof 守卫——T-102 落表后 probe 不抛 ReferenceError；③新增 `__LEVEL_INDEX`/`__WORLD_THEMES` 桥（v1 源为 null，T-102 落表自动桥上）；④`__isV2()` 运行时探测（`typeof LEVELS['1-1']`）。v1.9.0 源行为零变化。新增自检 `tests/playtests/v20-harness-compat.js`：**新 harness 21/21 全绿 + 旧 harness（HEAD 版临时导出）3/3 判别必红双重结论成立** | 程基岩（engineering-lead） |
