...
> 本子节为**手写复核段**，与 §7 自动生成段并列存档，用户拍板「双机并列」。**§7 内部（含 `<!-- PERF-BENCH:AUTO -->` 区）保持参考机口径不动**，本子节只作追加、不改 §7 任何字节。

- **环境**：Node v22.22.2 · win32 x64 · **Intel(R) Core(TM) i5-7500T CPU @ 2.70GHz × 4**
- **复测日期**：2026-09-21（UTC 12:14，本机低配机）
- **口径**：与 §7.**完全一致**（`node tests/perf/bench.js` 无头计数 ctx，每场景 100 帧预热 + 600 帧采样，seed=20260918，dt 恒定 16.7ms）

| 场景 | p50 (ms) | p95 (ms) | max (ms) | mean (ms) | p95 预算占比 | draw calls 全口径(中位) |
|---|---|---|---|---|---|---|
| A · L1 空场 | 0.11 | 0.22 | 1.50 | 0.13 | 1% | 290 |
| B · L1 中期 | 0.31 | 0.57 | 15.52 | 0.38 | 3% | 876 |
| C · L3 末波 | 0.54 | 1.00 | 4.23 | 0.63 | 6% | 1589 |
| D · L4 末波地狱 | 0.70 | 1.47 | 7.82 | 0.84 | 9% | 1889 |
| E · L5 末波地狱 | 1.07 | 2.21 | 16.25 | 1.29 | 13% | 2620 |

- **帧异常**：0　·　**判级**：PASS。
- **结论**：低配机（i5-7500T × 4，约为参考机 i5-12500 的 1/3 算力）上五场景**全 PASS**，最高 p95 占 16.7ms 帧预算的 **13%–22%**（两样本区间，波动详见下方披露段）；**draw call 数量与结构量级与 §7 参考机完全一致**（同场景全口径中位逐值相等：290 / 876 / 1589 / 1889 / 2620），说明帧耗时差异**纯属 CPU 算力，非渲染负载漂移**。

> **复测波动披露（本会话独立复核，如实记录）**：本子节落地时于同一低配机**再独立重跑一次 bench**，结果为——draw calls 全口径中位**逐场景完全一致**（290 / 876 / 1589 / 1889 / 2620），p50 逐场景吻合（偏差 ≤ 0.06ms），仅 p95/max 因 4 核低配主机负载呈**样本间波动**：该次重跑最坏为 E p95 3.68ms（22% 预算），帧异常 0，**仍判 PASS 且远低于 16.7ms 帧预算**。即：**p50 与 draw-call 结构结论稳定可复现**；p95/max 应读作受主机负载影响的**上界包线**，两种样本下均不触预算红线。
...
- **测试日期**：2026-09-22 07:24 UTC　·　**任务**：V11-11（KNOWN-ISSUES #8 回填）· 作者：程基岩
...
|---|---|---|---|---|---|---|---|---|---|
| A · L1 空场（开局无实体） | 0.05 | 0.10 | 0.31 | 0.06 | 1% | 290 | 194 | 155 | +25.2% |
| B · L1 中期（6 植物 + 10 僵尸） | 0.13 | 0.23 | 0.81 | 0.14 | 1% | 876 | 592 | 605 | -2.1% |
| C · L3 末波（12 植物 + 25 僵尸含 bucket） | 0.21 | 0.41 | 1.16 | 0.25 | 2% | 1590 | 1092 | 1009 | +8.2% |
| D · L4 末波地狱（15 植物 + 30 僵尸含双桶，水景） | 0.27 | 0.51 | 0.72 | 0.31 | 3% | 1890 | 1250 | — | —（新场景无估算） |
| E · L5 末波地狱（15 植物 + 30 僵尸含双桶，屋顶） | 0.43 | 0.76 | 3.36 | 0.48 | 5% | 2635 | 1356 | — | —（新场景无估算） |
