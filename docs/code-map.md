...
> 源文件：`plants-vs-zombies.html`（2494 行 · 69 个顶层函数 · 54 个顶层常量 · 11 个分区）
...
| 渲染 | L1331–L2118 | 788 | 16 |
| D-11 圆角化（蓝图 §6） | L2119–L2491 | 373 | 8 |
...
| 函数 | `drawGameWorld` | L1423–L1667 | — |
| 函数 | `drawCorpsePart` | L1668–L1685 | F-02 死亡零件绘制（蓝图 §3）：head 带眼睛，其余为矩形；末 300ms 淡出 |
| 函数 | `drawShovelIcon` | L1686–L1710 | 铲子图标（供铲子槽 / 光标预览复用） |
| 常量 | `POT_LIFT` | L1711–L1711 | 花盆自身下沉 POT_SINK px 落到格底——错位后盆口沿/盆身/盆底全部可见，植物底缘正好立在盆口，还原"种在盆里"的层次。 几何：豌豆底… |
| 常量 | `onPot` | L1712–L1713 | — |
| 函数 | `drawPlant` | L1714–L1756 | — |
| 函数 | `drawPlantInner` | L1757–L1945 | 原 drawPlant 绘制主体（阴影/各类型/血条），签名改为 (p, x, y) 以支持种植动画缩放平移 |
| 函数 | `drawZombie` | L1946–L2008 | — |
| 函数 | `drawProjectile` | L2009–L2049 | — |
| 函数 | `drawSun` | L2050–L2073 | — |
| 函数 | `drawParticle` | L2074–L2083 | — |
| 函数 | `drawShockwave` | L2084–L2102 | F-03 冲击波环（蓝图 §4）：easeOutCubic 扩散 8→180px，alpha 0.9→0，线宽 3→1.5 e.water（v1… |
| 函数 | `drawBoom` | L2103–L2121 | F-03 火球扩为 450ms 三色段（蓝图 §4）：白心→橙→红橙→透明，前 30% 涨后回缩 |

### D-11 圆角化（蓝图 §6） · L2119–L2491

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 常量 | `RADIUS` | L2122–L2122 | 统一圆角矩形：现代浏览器 ctx.roundRect 原生支持，退化 arcTo 兼容旧内核。 rr 内不碰 alpha；调用方需半透明时自行 … |
| 函数 | `rr` | L2123–L2135 | — |
| 函数 | `drawCardBar` | L2136–L2185 | — |
| 函数 | `drawCardFace` | L2186–L2285 | — |
| 函数 | `drawStatus` | L2286–L2330 | — |
| 函数 | `drawMenu` | L2331–L2408 | — |
| 函数 | `drawPause` | L2409–L2419 | — |
| 函数 | `drawEnd` | L2420–L2491 | — |
