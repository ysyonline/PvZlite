...
| v2.0 关卡数据层（T-102 · production/v2.0-plan.md §3.1-3.3） | L126–L673 | 548 | 17 |
| v1.4 元进度存档（T-02 · 积分/卡槽/卡池/卡组/通关计数） | L674–L751 | 78 | 6 |
| v1.4 结算（T-07 难度乘算 / T-08 通关奖励） | L752–L771 | 20 | 2 |
| v1.4 结算入口（T-06 · 胜利 sweep / 失败部分结算） | L772–L889 | 118 | 19 |
| 音效（WebAudio 实时合成，无外部文件） | L890–L1097 | 208 | 18 |
| 公共依赖 · Easing 缓动库（F-01/F-03/F-04 引用，feel-impl-skeleton §8） | L1098–L1213 | 116 | 5 |
| 主循环 | L1214–L1259 | 46 | 5 |
| 输入 | L1260–L1310 | 51 | 4 |
| 种植校验规则表（P1-A · code-review-todo 2026-09-20） | L1311–L1463 | 153 | 4 |
| v1.4 卡槽解锁（T-11 · 验收变更 2026-09-21：面板/商城入口下线，buySlot 逻辑保留备解冻） | L1464–L1691 | 228 | 8 |
...
### v2.0 关卡数据层（T-102 · production/v2.0-plan.md §3.1-3.3） · L126–L673
...
| 常量 | `CARDS` | L657–L680 | 植物卡 |

### v1.4 元进度存档（T-02 · 积分/卡槽/卡池/卡组/通关计数） · L674–L751

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 常量 | `points` | L681–L683 | 存量卡池推导仍走旧口径：poolFromProgress 按 unlocked 序位等价展开（T-105 切 cleared 集合 + 新表）。… |
| 常量 | `diffClears` | L684–L684 | v1.5 决议2：per-difficulty 通关记录（'难度:关号' → true）；无历史记录=存量玩家重通领取（patch note 披… |
| 函数 | `loadMeta` | L685–L737 | — |
| 函数 | `poolFromProgress` | L738–L745 | （'w-l' 真键直查）。旧数字序位展开退役。 旧档迁移等价性：N=2/3 与旧推导等价；N=5 已知差异——v2.0 序列重排（1-3 mel… |
| 函数 | `defaultDeck` | L746–L750 | — |
| 函数 | `inDeck` | L751–L757 | — |

### v1.4 结算（T-07 难度乘算 / T-08 通关奖励） · L752–L771

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 函数 | `computeClearReward` | L758–L768 | 纯函数（无副作用，只读 saveCleared/settleRun 传入的通关前快照语义由调用点保证）； 本期世界 1 无法凑满 10 键（35… |
| 函数 | `settlePointsRaw` | L769–L779 | 局合计取整口径（T-07）：(击杀收集合计 + 通关奖励) × DIFFS[DIFF].mult → Math.round 一次。 不做 per… |

### v1.4 结算入口（T-06 · 胜利 sweep / 失败部分结算） · L772–L889

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 常量 | `endStats` | L780–L780 | 「通关前快照」（saveCleared 入集之前），首通发、重通零发（幂等，无需新存档字段）； 与 worldClear 一并进 settleP… |
| 函数 | `settleRun` | L781–L801 | — |
| 函数 | `saveMeta` | L802–L823 | — |
| 常量 | `state` | L824–L825 | 状态 |
| 常量 | `highScore` | L826–L826 | — |
| 常量 | `plants` | L827–L829 | — |
| 函数 | `setState` | L830–L838 | 状态切换统一入口（带控制台留痕，方便排查“突然退出对局”这类偶现问题） |
| 函数 | `updateBest` | L839–L842 | 历史最高分落盘（仅当刷新纪录时写，减少 localStorage 写入，2026-09-16 #9） |
| 常量 | `toastMsg` | L843–L843 | 屏幕提示条（临时消息） |
| 函数 | `toast` | L844–L844 | — |
| 函数 | `drawToast` | L845–L862 | — |
| 常量 | `exitArm` | L863–L863 | 对局中返回菜单需要二次确认，避免误触/误按丢掉进度 |
| 函数 | `requestExit` | L864–L871 | — |
| 常量 | `R` | L872–L872 | 工具 |
| 常量 | `C` | L873–L875 | — |
| 函数 | `liftX` | L876–L879 | v1.3-M4 C2 屋顶抬升函数：返回该 x 处的抬升量（≥0 = 向上 = 屏幕 y 减小）。 ★ 单点 gate：!level.roof … |
| 函数 | `gridToPos` | L880–L883 | — |
| 函数 | `posToGrid` | L884–L887 | — |
| 函数 | `inGrid` | L888–L890 | — |

### 音效（WebAudio 实时合成，无外部文件） · L890–L1097

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 常量 | `audioCtx` | L891–L891 | — |
| 常量 | `audioQueue` | L892–L892 | — |
| 函数 | `ac` | L893–L903 | — |
| 函数 | `scheduleOrDefer` | L904–L907 | 排程守卫：上下文未 running（suspended/interrupted）时，直接排程的事件可能被整段丢弃 （用户反馈 2026-09-1… |
| 函数 | `flushAudioQueue` | L908–L916 | — |
| 函数 | `primeAudio` | L917–L925 | 音频管线预热：上下文刚创建时音频线程尚未就绪，会话首个音效常被整段吞掉 （用户反馈 2026-09-16：开局种植物没声音，之后的音效正常）。 … |
| 函数 | `armAudioUnlock` | L926–L947 | 兜底：任意一次用户手势都尝试恢复音频上下文（部分浏览器创建后仍保持 suspended） |
| 常量 | `BGM` | L948–L957 | + 低音层（sine 根音），C 大调五声，C-G-Am-F 和声进行。 音量按「经 env 总线 0.5 衰减后仍清晰可闻」标定（初版 0.0… |
| 函数 | `updateBGM` | L958–L967 | — |
| 函数 | `bgmTick` | L968–L999 | — |
| 常量 | `AudioBus` | L1000–L1006 | ---- 音频总线（ADR-004）：4 分组 GainNode + masterGain → destination ---- 分组默认音量：… |
| 常量 | `AUDIO_ROUTES` | L1007–L1012 | SFX 分组映射（tone/noise 默认走 event；战斗/UI 音效按需显式传分组）。 本表是「规格侧」的权威映射，运行时由各 SFX … |
| 函数 | `initAudioBus` | L1013–L1045 | — |
| 函数 | `tone` | L1046–L1050 | 单音：频率、时长、波形、音量、延迟、滑到目标频率 |
| 函数 | `scheduleTone` | L1051–L1064 | — |
| 函数 | `noise` | L1065–L1069 | 噪声：用于爆炸/挖掘/啃食。按 dur 就近取预生成 buffer 段（0.1/0.3/0.5s） |
| 函数 | `scheduleNoise` | L1070–L1092 | — |
| 函数 | `routeBus` | L1093–L1098 | 取输出总线：group 显式优先（SFX 调用点已标注所属分组）；缺省走 event。 总线未初始化（无头测试 / 降级）时直连 destina… |

### 公共依赖 · Easing 缓动库（F-01/F-03/F-04 引用，feel-impl-skeleton §8） · L1098–L1213

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 常量 | `Easing` | L1099–L1112 | — |
| 常量 | `sfxGate` | L1113–L1113 | 节流：防止密集事件把音频糊成一团 |
| 函数 | `gate` | L1114–L1118 | — |
| 常量 | `SFX` | L1119–L1192 | — |
| 常量 | `SirenLoop` | L1193–L1214 | 采用「帧驱动」而非 setInterval： · 暂停时 update 不执行 → 警报天然同步暂停（§C.1 的暂停要求零额外代码） · 横幅… |

### 主循环 · L1214–L1259

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 常量 | `lastT` | L1215–L1215 | — |
| 常量 | `lastDt` | L1216–L1216 | — |
| 常量 | `frameErr` | L1217–L1217 | — |
| 函数 | `loop` | L1218–L1248 | — |
| 函数 | `drawFrameErr` | L1249–L1260 | — |

### 输入 · L1260–L1310

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 常量 | `lastMouseGrid` | L1261–L1274 | — |
| 函数 | `bindBtn` | L1275–L1280 | 按钮统一绑定：点完主动 blur，避免按钮保留焦点后被 Space/Enter 二次触发 |
| 函数 | `togglePause` | L1281–L1294 | 暂停切换统一入口：三处触发（按钮/空格/Esc）收敛到此，切换后同步 BGM 起停 （2026-09-20 用户反馈：暂停后 BGM 还在放——… |
| 函数 | `applyMuteUI` | L1295–L1319 | 静音按钮外观跟随 muted 状态（启动即按存档恢复，2026-09-16 #9） |

### 种植校验规则表（P1-A · code-review-todo 2026-09-20） · L1311–L1463

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 函数 | `canPlant` | L1320–L1339 | ③垫/盆不算占用（垫上/盆上可连种）；同载体重叠（垫上铺垫/盆上放盆）与普通重复种植算占用。 ④水轴是单条强约束「水域关+水行」双条件放行（v1… |
| 函数 | `spawnPlant` | L1340–L1351 | 旧对象兜底（drawPlant L1748 区 plantT===undefined）保留不删——harness 注入的测试对象 仍是旧 sch… |
| 函数 | `onClick` | L1352–L1457 | — |
| 函数 | `onClickMenu` | L1458–L1464 | — |

### v1.4 卡槽解锁（T-11 · 验收变更 2026-09-21：面板/商城入口下线，buySlot 逻辑保留备解冻） · L1464–L1691

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 函数 | `buySlot` | L1465–L1474 | — |
| 常量 | `DECK_GRID` | L1475–L1475 | v1.4 T-10 选卡界面（state='deck'，验收变更 2026-09-21：两排布局）： 上排 = 当前可选植物（ownedCard… |
| 常量 | `DECK_SLOTS` | L1476–L1476 | — |
| 函数 | `drawSelectDeck` | L1477–L1556 | — |
| 函数 | `onClickSelect` | L1557–L1589 | — |
| 函数 | `onClickDeck` | L1590–L1616 | — |
| 函数 | `onClickEnd` | L1617–L1636 | — |
| 函数 | `onKey` | L1637–L1692 | — |

### 游戏控制 · L1692–L1714

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 函数 | `startGame` | L1693–L1716 | — |

### 波次 · L1715–L1792

| 类型 | 名称 | 行号区间 | 说明 |
|---|---|---|---|
| 常量 | `spawnQueue` | L1717–L1717 | 波次生成：用队列逐个放出，不是一次性全刷 |
| 常量 | `spawnTimer` | L1718–L1718 | — |
| 常量 | `waveInterval` | L1719–L1719 | — |
| 常量 | `waveActive` | L1720–L1720 | — |
| 常量 | `waveDrainedT` | L1721–L1724 | — |
| 常量 | `spawnGateZ` | L1725–L1725 | v1.3-M4 FIX-01 v2（口径修订 2026-09-19 20:52）：教学关（'1-1'/'1-2'；T-104b 前为数字关号 1… |
| 常量 | `spawnGateT` | L1726–L1728 | — |
| 常量 | `warn` | L1729–L1730 | 大波预警：active=横幅显示中，t=剩余秒数，last=已预警过的波次号， pending=倒计时已结束但还在等场上清空（横幅此时已隐藏，只… |
| 函数 | `newWave` | L1731–L1767 | — |
| 函数 | `processSpawnQueue` | L1768–L1793 | 从队列中逐个放出僵尸 |
