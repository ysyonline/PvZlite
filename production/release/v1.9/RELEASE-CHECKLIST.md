...
| # | 项 | 状态 |
|---|---|---|
| 1 | `v1.9.0` 轻量标签 + main 推送远端 | ✅ 已授权执行中：推后 `git ls-remote` 双确认 + 冻结副本 blob 远端比对 |
| 2 | **R7 脚本修复**（`v18-splash-distribution.js` 读 `e.w.kind` 复跑必红） | ⏳ 待拍板——R10 报告 §5 登记的潜在回归；R10/R11 新管线不受影响，建议下版本修复并补判别力自证 |
